...
def summarize_top(df: pd.DataFrame, value_col: str, n: int, ascending: bool = False) -> pd.DataFrame:
    return df.sort_values(value_col, ascending=ascending).head(n).reset_index(drop=True)
...
class DepMapProvider:
    ...
    def _load_matrix(self, path: str, context: str) -> pd.DataFrame:
        ...
        # Convert everything possible to numeric; keep only columns with at least one numeric value
...
def summarize_mutations(mut_df: pd.DataFrame, provider: DepMapProvider) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    if mut_df.empty:
...
def main():
    ...
    parser.add_argument("--expression-file", required=True, help="DepMap expression CSV")
    parser.add_argument("--mutation-file", required=True, help="DepMap mutations CSV")
    parser.add_argument("--copy-number-file", required=True, help="DepMap copy number CSV")
    parser.add_argument("--essentiality-file", required=True, help="DepMap CRISPR gene effect CSV")
    parser.add_argument("--metadata-file", required=True, help="DepMap model metadata CSV")
    ...
    coexpr_df = compute_correlations_to_all_genes(expr_matrix, gene=gene, method=args.corr_method, top_n=args.top_n)
    ...
    coess_df = compute_correlations_to_all_genes(dep_matrix, gene=gene, method=args.corr_method, top_n=args.top_n)
    ...
    with open(os.path.join(args.outdir, "summary.txt"), "w", encoding="utf-8") as f:
        ...
        f.write(f"Gene: {row['query_gene']}\n")
        f.write(f"Expression max: {row['expression_max']}\n")
        f.write(f"Expression median: {row['expression_median']}\n")
        f.write(f"Expression detected fraction (>1): {row['expression_detected_fraction_gt1']}\n")
        f.write(f"Mutation rows: {row['mutation_rows']}\n")
        f.write(f"Copy number max: {row['copy_number_max']}\n")
        f.write(f"Copy number min: {row['copy_number_min']}\n")
        f.write(f"Amplified fraction: {row['copy_number_amplified_fraction']}\n")
        f.write(f"Deleted fraction: {row['copy_number_deleted_fraction']}\n")
        f.write(f"Essentiality min: {row['essentiality_min']}\n")
        f.write(f"Essentiality median: {row['essentiality_median']}\n")
        f.write(f"Essential fraction (<{args.essential_threshold}): {row['essential_fraction_lt_threshold']}\n")
        f.write(f"Top co-expressed gene: {row['top_coexpressed_gene']} ({row['top_coexpressed_corr']})\n")
        f.write(f"Top co-essential gene: {row['top_coessential_gene']} ({row['top_coessential_corr']})\n")
